...
const projectImages = [project1Img, project2Img, project3Img, project1Img];
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mb-16">
					{unifiedData.projects.map((project, index) => (
						<motion.div
							key={project.title}
							className="rounded-xl shadow-lg overflow-hidden bg-gray-800 text-gray-300 border border-gray-700"
									...
									<div className="grid md:grid-cols-2 gap-10">
										...
										<div className="flex flex-col items-center">
											...
											<div className="flex justify-center gap-6 mt-2">
												{unifiedData.projects[selectedProject].github && (
													<a href={unifiedData.projects[selectedProject].github} target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:text-cyan-200 transition-colors text-2xl">
														<FiGithub />
													</a>
												)}
												{unifiedData.projects[selectedProject].demo && (
													<a href={unifiedData.projects[selectedProject].demo} target="_blank" rel="noopener noreferrer" className="text-purple-400 hover:text-purple-200 transition-colors text-2xl">
														<FiExternalLink />
													</a>
												)}
											</div>
										...
										<div>
											{/* Features */}
											{unifiedData.projects[selectedProject].features && (
												<div className="mb-6">
													<h4 className="text-xl font-semibold text-cyan-200 mb-3">Key Features</h4>
													<ul className="space-y-2">
														{unifiedData.projects[selectedProject].features.map((feature, idx) => (
											...
											{/* Awards/Tags */}
											{unifiedData.projects[selectedProject].tags && unifiedData.projects[selectedProject].tags.length > 0 && (
												<div className="mb-6">
													<h4 className="text-xl font-semibold text-yellow-300 mb-3">Awards & Tags</h4>
													<div className="flex flex-wrap gap-2">
														{unifiedData.projects[selectedProject].tags.map((tag, idx) => (
															<span key={tag} className="px-3 py-1 rounded-full bg-yellow-400/20 text-yellow-200 text-xs font-semibold shadow">
																{tag}
															</span>
														))}
													</div>
